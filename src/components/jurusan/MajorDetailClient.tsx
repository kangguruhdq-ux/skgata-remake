...
interface MajorDetailClientProps {
  initialMajor: MajorData;
  slug: string;
}

export default function MajorDetailClient({ initialMajor, slug }: MajorDetailClientProps) {
  const { majors, teachers } = useCMS();

  // Dynamically resolve major from active CMS store with fallback to initial static data
  const major = majors.find((m) => m.slug === slug) || initialMajor;

  // Filter teachers belonging to this major's department
  const majorTeachers = teachers.filter((t) => {
    const dept = (t.department || "").toLowerCase();
    const role = (t.role || "").toLowerCase();
    const mName = major.name.toLowerCase();
    const mCode = major.code.toLowerCase();
    return (
      dept.includes(mName) ||
      dept.includes(mCode) ||
      role.includes(mName) ||
    ...
    <div className="bg-slate-50 py-10 lg:py-16">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              <div className="pt-2 flex flex-wrap gap-3 text-xs text-slate-300">
                ...
                <span className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-xl border border-white/10 backdrop-blur-sm">
                  ...
                  <span>{major.gallery?.length || 0} Galeri Bengkel</span>
                </span>
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          {/* Left Column: Competencies, Gallery, Facilities, Works, Teachers */}
          <div className="lg:col-span-8 space-y-8">
            ...
            {major.gallery && major.gallery.length > 0 && (
            ...
            {major.studentWorks && major.studentWorks.length > 0 && (
            ...
            {/* 5. DAFTAR TIM GURU & PENDIDIK KEJURUAN */}
            {majorTeachers.length > 0 && (
              <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm interactive-card reveal-left delay-4">
                <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-100">
                  <div className="flex items-center gap-2 text-skagata-700">
                    <Users className="w-5 h-5 text-emerald-600" />
                    <h2 className="font-display font-bold text-lg sm:text-xl text-slate-900">
                      Tim Guru & Pendidik {major.name}
                    </h2>
                  </div>
                  <span className="text-xs bg-emerald-50 text-emerald-700 font-bold px-3 py-1 rounded-full border border-emerald-200">
                    {majorTeachers.length} Guru Pengampu
                  </span>
                </div>

                <div className="grid sm:grid-cols-2 gap-3.5">
                  {majorTeachers.map((teacher) => (
                    <div
                      key={teacher.id}
                      className="p-3 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center gap-3 hover:bg-emerald-50/30 transition"
                    >
                      <div className="w-12 h-14 rounded-xl overflow-hidden bg-slate-100 shrink-0 border border-slate-200">
                        <img
                          src={teacher.photo || DEFAULT_TEACHER_PHOTO}
                          alt={teacher.name}
                          className="w-full h-full object-cover"
                        />
                      </div>
                      <div className="min-w-0 flex-1">
                        <h4 className="font-display font-bold text-xs text-slate-900 truncate" title={teacher.name}>
                          {teacher.name}
                        </h4>
          ...
          <div className="lg:col-span-4 space-y-6 reveal-right delay-2">
            {/* Profil Ketua Konsentrasi (DYNAMIC FROM CMS / ADMIN) */}
            <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm text-center">
              <div className="relative w-24 h-24 rounded-2xl overflow-hidden mx-auto shadow-md border-2 border-emerald-500/40 mb-3 bg-slate-100">
                <img
                  src={
                    major.headOfMajorPhoto ||
                    DEFAULT_TEACHER_PHOTO
                  }
                  alt={major.headOfMajor || "Ketua Konsentrasi"}
                  className="w-full h-full object-cover"
                />
              </div>

              <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full inline-block">
                Ketua Konsentrasi Keahlian
              </span>
              <h3 className="font-display font-bold text-base text-slate-900 mt-1.5" title={major.headOfMajor}>
